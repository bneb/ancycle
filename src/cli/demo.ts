...
async function main() {
  ...
  if (node2.fromTable === node1.tableName) {
      ...
      const allVariables = { ...node1.variables, ...node2.variables };
      for (const [name, type] of Object.entries(allVariables)) {
          console.log(`  (declare-const ${name} ${type})`);
      }
      ...
      try {
        const result = await checkZ3Constraints(allAssertions, allVariables);
