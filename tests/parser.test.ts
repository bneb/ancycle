...
function parse(sql: string) {
    return TreeSitterAdapter.parseSelectStatement(sql);
}
...
WHERE
  status = 'active'`;
    ...
    test("parses single CTE", () => {
        ...
        `;
        const stmt = parse(sql);
        expect(stmt.withClauses).toBeDefined();
        expect(stmt.withClauses!.length).toBe(1);
        expect(stmt.withClauses![0].name).toBe("active_users");
        expect(stmt.withClauses![0].stmt.fromTable).toBe("users");
        expect(stmt.fromTable).toBe("active_users");
    });
    ...
    test("parses multiple CTEs", () => {
        ...
        const stmt = parse(sql);
        expect(stmt.withClauses).toBeDefined();
        expect(stmt.withClauses!.length).toBe(2);
        expect(stmt.withClauses![0].name).toBe("cte1");
        expect(stmt.withClauses![1].name).toBe("cte2");
        expect(stmt.fromTable).toBe("cte1");
    });
});
