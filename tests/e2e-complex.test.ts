...
describe("E2E Complex Query", () => {
    ...
    test("parser parses the complex query without throwing", () => {
        const stmt = TreeSitterAdapter.parseSelectStatement(COMPLEX_QUERY);
    ...
    test("parses GROUP BY and HAVING", () => {
        ...
        // Not fully parsing group by and having yet in tree-sitter-adapter but let's test it doesn't crash
        const stmt = TreeSitterAdapter.parseSelectStatement(sql);
    });

    test("parses ORDER BY and LIMIT", () => {
        const sql = "SELECT * FROM users ORDER BY name DESC LIMIT 10";
        // Not fully parsing order by and limit yet in tree-sitter-adapter but let's test it doesn't crash
        const stmt = TreeSitterAdapter.parseSelectStatement(sql);
    });
});
