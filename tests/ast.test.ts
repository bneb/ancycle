...
import { Identifier, StringLiteral, NumberLiteral, BinaryExpr, SelectStmt, CaseExpr } from "../src/parser/ast";
...
describe("AST Nodes", () => {
    ...
    test("CaseExpr produces nested (ite ...) logic", () => {
        const caseExpr = new CaseExpr(
            [
                { condition: new BinaryExpr(new Identifier("age"), "<", new NumberLiteral(18)), result: new StringLiteral("minor") },
                { condition: new BinaryExpr(new Identifier("age"), ">=", new NumberLiteral(18)), result: new StringLiteral("adult") }
            ],
            new StringLiteral("unknown")
        );
        const smt = caseExpr.toSmtLib();
        expect(smt).toBe('(ite (< age 18) "minor" (ite (>= age 18) "adult" "unknown"))');
    });

    test("CaseExpr without else branch defaults to NULL", () => {
        const caseExpr = new CaseExpr(
            [{ condition: new BinaryExpr(new Identifier("x"), "=", new NumberLiteral(1)), result: new StringLiteral("one") }],
            null
        );
        expect(caseExpr.toSmtLib()).toBe('(ite (= x 1) "one" "NULL")');
    });
});
